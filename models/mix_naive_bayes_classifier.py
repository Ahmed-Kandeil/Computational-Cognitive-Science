...
def calculate_class_probability(
    df: pd.DataFrame,
    gaussian_cols: list[str],
    exponential_cols: list[str],
    target_col: str,
    values: list[float],
    std: float = None,
) -> np.ndarray:
    ...
    Parameters:
    - df: DataFrame, input data
    - gaussian_cols: list of str, Gaussian feature columns
    - exponential_cols: list of str, Exponential feature columns
    - target_col: str, the target column
    ...
    feature_cols = sorted(gaussian_cols + exponential_cols)

    individual_probabilities = [
        exponential_pdfs(df, col, target_col, value)
        if col in exponential_cols
        else gaussian_pdfs(df, col, target_col, value, std)
...
def mix_naive_bayes_classifier(
    df: pd.DataFrame,
    gaussian_cols: list[str],
    exponential_cols: list[str],
    target_col: str,
